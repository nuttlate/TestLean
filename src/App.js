...
const App = () => {
  ...
  useEffect(() => {
    if (!isAuthReady || !db) return; // userId is not needed for public collection query
    ...
    return () => unsubscribe();
  }, [isAuthReady]); // Re-run when auth state changes (userId not a dependency for public data)

  const handleDateClick = (date) => {
    setSelectedDate(date);
    // Always open modal for adding a new event when clicking a date on the calendar
    setEventToEdit(null);
    setIsModalOpen(true);
  };
  ...
  const handleSaveEvent = async (eventData, eventId = null) => {
    if (!db) {
      // userId is not needed for public collection write
  ...
  const handleDeleteEvent = async (eventId) => {
    if (!db) {
      // userId is not needed for public collection delete
      console.error("Firestore not available.");
      return;
    }
